...
type NonDefaultType = Exclude<LabType, "default">;
type NonDefaultPropsByType = {
  "book-to-limited-series": BookToLimitedSeriesProps;
  "work-to-series-adaptation": WorkToSeriesAdaptationProps;
  "palmylyzer-pro": PalmylyzerProProps;
  "song-recording": SongRecordingProps;
};
type CommonBase = Pick<
  DefaultProps,
...
type NonDefaultRegistry = {
  [K in NonDefaultType]: (context: BuilderContext) => NonDefaultPropsByType[K];
};

const KNOWN_TYPES: readonly LabType[] = [
  "default",
...
const normalizeOrientation = (value: unknown): MovieOrientation =>
...
const createMissingFieldError = (
  ...
  return new Error(
    `[ai-studio] Missing required field "${fieldName}" for type "${labType}" on item "${slug}".`,
  );
};
...
const resolveLabType = (value: unknown): LabType => {
  if (typeof value === "string" && KNOWN_TYPES.includes(value as LabType)) {
    return value as LabType;
  }
...
const getItemTextBlob = (item: DataItem): string =>
  [
...
const hasVideoMedia = (item: DataItem): boolean => {
...
const resolveMediumTags = (item: DataItem, labType: LabType) => {
...
const resolveStyleTags = (item: DataItem, labType: LabType) => {
...
const buildCommonBaseProps = (item: DataItem, rank: number): CommonBase => {
  return {
    rank,
    title: item.title,
...
const nonDefaultRegistry: NonDefaultRegistry = {
...
const buildDefaultProps = (context: BuilderContext): DefaultProps => {
...
export const buildLabProps = (item: DataItem, rank: number): LabProps => {
  const itemRecord = item as Record<string, unknown>;
  ...
  const labType = resolveLabType(item.type);
...
export const normalizeItems = (
  ...
  return items.map((item, index) => {
    const props = buildLabProps(item, index + 1);
    const labType = resolveLabType(item.type);
